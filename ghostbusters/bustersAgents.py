    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        maxBeliefPosition = [max(dist, key = lambda x: dist[x]) for dist in livingGhostPositionDistributions]
        minAct = None
        minVal = float('inf')
        for action in legal:
            newPos = Actions.getSuccessor(pacmanPosition,action)
            val = min(self.distancer.getDistance(newPos,ghostPos) for ghostPos in maxBeliefPosition)
            if (val < minVal):
                minAct = action
                minVal = val
        return minAct
